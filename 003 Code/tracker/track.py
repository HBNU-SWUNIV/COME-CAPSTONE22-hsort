...
def run(
        ...
        strong_sort_weights=WEIGHTS / 'osnet_ibn_x1_0.pt',  # model.pt path,
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                t4 = time_sync()
                outputs[i], update_dict = strongsort_list[i].update(xywhs.cpu(), confs.cpu(), clss.cpu(), im0)  ###
                db.update_db(update_dict)   ###
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            label = f'id={id:0.0f} location={location:d}'    ###
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=str, default=WEIGHTS / 'yolov5x6.pt', help='model.pt path(s)')
    parser.add_argument('--strong-sort-weights', type=str, default=WEIGHTS / 'resnet50_fc512_msmt17.pt')
